...
DtmAdaptive::DtmAdaptive(const PerformanceCounters *perf_counters,
                         ...
                         int freq_step_mhz, float t_recover, int k_max,
                         float slack_scale, float mem_intensity_threshold,
                         float mpki_threshold, int freq_history_len)
    : m_perf(perf_counters), m_num_cores(num_cores), m_cores_in_x(cores_in_x),
      ...
      m_freq_step(freq_step_mhz), m_k_max(k_max), m_slack_scale(slack_scale),
...
bool DtmAdaptive::isThrashing(CoreStats coreStats) const {
  ...
  if (thrash) {
    if (m_log.is_open())
      m_log << "[DTM-Adaptive] thrashing-check"
            << " ipc=" << coreStats.ipc
...
DtmAdaptive::getDecisions(const std::vector<int> &core_thread_running,
  ...
  for (int i = 0; i < m_num_cores; ++i) {
    ...
    } else if (is_hp && isThrashing(stats)) {
