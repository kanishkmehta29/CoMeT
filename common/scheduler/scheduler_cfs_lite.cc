...
SchedulerCFSLite::SchedulerCFSLite(ThreadManager *thread_manager)
   ...
   , m_lb_enable(Sim()->getCfg()->getBool("scheduler/cfs_lite/load_balance/enable"))
   , m_lb_epoch(SubsecondTime::NS(Sim()->getCfg()->getInt("scheduler/cfs_lite/load_balance/epoch")))
   , m_lb_imbalance_threshold((UInt64)std::max<SInt64>(0, Sim()->getCfg()->getInt("scheduler/cfs_lite/load_balance/imbalance_threshold")))
   , m_lb_max_migrations_per_epoch((UInt64)std::max<SInt64>(0, Sim()->getCfg()->getInt("scheduler/cfs_lite/load_balance/max_migrations_per_epoch")))
...
UInt64 SchedulerCFSLite::getCoreLoad(core_id_t core_id) const
{
   ...
   UInt64 load = (UInt64)m_core_runqueues[core_id].size();
   if (core_id < (core_id_t)m_core_thread_running.size() && m_core_thread_running[core_id] != INVALID_THREAD_ID)
      ++load;
 ...
 */
thread_id_t SchedulerCFSLite::pickMigrationCandidate(core_id_t from_core, core_id_t to_core) const
{
   if (from_core >= (core_id_t)m_core_runqueues.size() || to_core >= (core_id_t)m_core_thread_running.size())
   ...
   for (std::deque<thread_id_t>::const_iterator it = rq.begin(); it != rq.end(); ++it)
   {
      ...
      if (tid >= (thread_id_t)m_threads_runnable.size() || !m_threads_runnable[tid])
         continue;
 ...
 *   slice_cycles = target_latency_ns * (thread_weight / total_weight) * freq_GHz
 * Clamped to at least min_granularity_ns * freq_GHz cycles.
 */
...
void SchedulerCFSLite::initDVFSPolicy(const String &logic)
{
   ...
   if (logic == "coreMemDTM") {
      ...
      m_dvfs_policy = new DVFSCoreMem(
          ...
          Sim()->getConfig()->getApplicationCores(),
          core_min_freq, core_max_freq, freq_step,
          dtmManager,
          dvfs_temp_margins, dvfs_frequencies,
          sb_temp_thresholds_C, sb_stall_counts_S,
          alpha, beta);
   }
   else {
      m_dvfs_policy = NULL;
...
void SchedulerCFSLite::initDtmPolicy(const String &logic)
{
   ...
   if (logic == "adaptive") {
      ...
      m_dtm_policy = new DtmAdaptive(
          ...
          num_channels, t_warn, t_crit, alpha_mem, min_freq, freq_step, t_recover,
...
bool SchedulerCFSLite::threadSetAffinity(thread_id_t calling_thread_id, thread_id_t thread_id,
...
void SchedulerCFSLite::handleLoadBalance(SubsecondTime time)
{
   ...
   while (migrations < m_lb_max_migrations_per_epoch)
   {
      ...
      std::cout << "[CFS-Lite][LB] migrate queued t" << (unsigned long long)candidate
                << " c" << (unsigned long long)busiest
                << " -> c" << (unsigned long long)idlest
                << " gap=" << (unsigned long long)(busiest_load - idlest_load)
                << std::endl;

      if (m_core_thread_running[idlest] == INVALID_THREAD_ID && hasRunnablePinnedThread(idlest))
...
void SchedulerCFSLite::handleDTM(SubsecondTime time)
{
   ...
   std::vector<DtmDecision> decisions = m_dtm_policy->getDecisions(running, weights, rq_empty);
   ...
   for (const DtmDecision &d : decisions)
   {
      ...
      case DtmAction::YIELD:
         if (m_dtm_enable_yield)
         {
            ...
            // Uncomment the block below to enable this penalty:
            //
